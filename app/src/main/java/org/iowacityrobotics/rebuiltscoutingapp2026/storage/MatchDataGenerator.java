...
public class MatchDataGenerator {
    ...
    // ===============================
// 7️⃣ Merge Practice Matches from Assets
// ===============================
    private static void mergePracticeMatches(Context context) {
        try {
            // Read internal file
            File file = new File(context.getFilesDir(), "match_data.json");
            ...
            fis.read(buffer);
            fis.close();
            JSONObject internalJson = new JSONObject(new String(buffer, StandardCharsets.UTF_8));
            JSONArray internalMatches = internalJson.getJSONArray("matches");
            ...
            is.close();
            JSONObject assetsJson = new JSONObject(baos.toString(StandardCharsets.UTF_8.name()));
            JSONArray assetsMatches = assetsJson.getJSONArray("matches");

            // Append only Practice matches from assets
            for (int i = 0; i < assetsMatches.length(); i++) {
                JSONObject match = assetsMatches.getJSONObject(i);
                if (match.getString("match_type").equalsIgnoreCase("Practice")) {
                    internalMatches.put(match);
                }
            }

            // Write merged result back to internal file
            JSONObject merged = new JSONObject();
            merged.put("matches", internalMatches);
            FileWriter writer = new FileWriter(file);
            writer.write(merged.toString(4));
            writer.close();

        } catch (IOException e) {
            // No assets file — skip silently
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
